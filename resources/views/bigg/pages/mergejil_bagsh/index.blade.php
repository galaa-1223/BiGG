...
@section('subcontent')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 lg:col-span-6">
            <form class="validate-form-teacher validate-form" action="{{ route('bigg-mergejil_bagsh-save') }}" method="post" enctype="multipart/form-data">
                @csrf
                <!-- BEGIN: Анги нэмэх -->
                <div class="intro-y box p-5">
                    <div class="input-form">
                        <label class="flex flex-col sm:flex-row">
                        Мэргэжлийн багш нэр: <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-gray-600">Криллээр бичнэ</span>
                        </label>
                        <input type="text" name="ner" placeholder="Компьютерийн багш" class="form-control w-full border mt-2" minlength="2" required data-pristine-minlength-message="2 тэмдэгдээс дээш байх ёстой" data-pristine-required-message="Багшийн мэргэжлийн нэр хоосон байж болохгүй"/>
                    </div>
            ...
            <table id="table" class="table table-report mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="whitespace-nowrap">Багшийн мэргэжлийн нэр</th>
                ...
                <tbody>
                        ...
                        <tr class="intro-x">
                            <td class="w-10">
                                <div class="flex">
                                <?=(($mergejil_bagshs->currentpage()-1) * $mergejil_bagshs->perpage() + $i);?>
                                </div>
                            </td>
                            <td>
                                <a href="#" class="font-medium whitespace-nowrap">{{ $mergejil_bagsh->ner }}</a>
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3" href="{{ url('bigg/mergejil_bagsh/edit/'.$mergejil_bagsh->id) }}">
                                        <i data-feather="check-square" class="w-4 h-4 mr-1"></i> {{ __('site.edit') }}
                                    </a>
                                    <a class="flex items-center text-theme-6 delete_button" href="javascript:;" data-id="{{ $mergejil_bagsh->id }}" data-target="#delete-confirmation-modal">
        ...
        <div class="modal-dialog"> 
            <div class="modal-content"> 
                <div class="modal-body p-0"> 
                    <form action="{{ route('bigg-mergejil_bagsh-delete-ajax') }}" method="post">
                    @csrf
                        <input type="hidden" class="t_id" name="t_id" value="">
                        <div class="p-5 text-center">
                            <i data-feather="x-circle" class="w-16 h-16 text-theme-6 mx-auto mt-3"></i>
                            <div class="text-3xl mt-5">Сонгосон багшийн мэргэжил устгахыг хүсэж байна уу?</div>
